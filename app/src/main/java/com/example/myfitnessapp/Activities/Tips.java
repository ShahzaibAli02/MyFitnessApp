...
public class Tips  extends AppCompatActivity {
    ...
    ArrayList<ModelTips> exerciseArrayList=new ArrayList<>();
    ...
    private void loadData()
    {
        ...
        FirebaseDatabase.getInstance().getReference("Tips").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot)
            {
                ...
                {
                    exerciseArrayList.add(snapshot.getValue(ModelTips.class));
                }

                if(exerciseArrayList.size()<1)
                {
                    Toast.makeText(Tips.this,"No Data Found",Toast.LENGTH_LONG).show();
                }
                else
                {

                    recyclerView.setAdapter(new TipsAdapter(Tips.this,exerciseArrayList));
